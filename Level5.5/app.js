document.addEventListener('DOMContentLoaded', () => {
    ...
    class Organism {
        ...
        update() {
            ...
            if (this.type === 'herbivore') {
                ...
                // 1. Flee from nearest predator within 75px (gives prey a running chance)
                let nearestPred = null;
                let minPredDistSq = 75 * 75;
                for (let i = 0; i < individuals.length; i++) {
                    const other = individuals[i];
                    if (other.type === 'predator' && other.alive) {
                        const dx = other.x - this.x;
                        const dy = other.y - this.y;
                        const distSq = dx * dx + dy * dy;
                        if (distSq < minPredDistSq) {
                            minPredDistSq = distSq;
                            nearestPred = other;
                        }
                    }
                }
                ...
                if (nearestPred) {
                    const dist = Math.sqrt(minPredDistSq);
                    if (dist > 0.1) {
                        // Strong fleeing vector away from predator
                        this.vx -= (nearestPred.x - this.x) / dist * 0.85;
                        this.vy -= (nearestPred.y - this.y) / dist * 0.85;
                    }
                } else if (!isSatiated && foodItems && foodItems.length > 0) {
                    // 2. Otherwise, steer towards nearest grass within 60px if not satiated
    ...
    function simulateGeneration(baseFoodSpawnRate, seasonLength, herbivoreCost, predatorCost) {
        ...
        const predatorNutritiveValue = 30;
        ...
        for (let step = 0; step < SUBSTEPS; step++) {
            ...
            for (let i = 0; i < individuals.length; i++) {
                ...
                if (pred.alive && pred.type === 'predator' && pred.huntCooldown <= 0 && !isSatiated) {
                    ...
                    for (let j = 0; j < individuals.length; j++) {
                        ...
                        if (prey.alive && prey.type === 'herbivore') {
                            ...
                            if (distSq < eatRadius * eatRadius) {
                                ...
                                pred.huntCooldown = 2; // Cooldown for 2 generations (prevents vacuuming)
        ...
        for (let i = 0; i < individuals.length; i++) {
            ...
            org.age++;
            if (org.huntCooldown > 0) org.huntCooldown--;
            
            let cost = org.type === 'predator' ? predatorCost : herbivoreCost;
            let isStressedPredator = false;

            if (org.type === 'predator') {
                // Predator Territoriality: Count other predators within 120px
                let neighborPreds = 0;
                for (let j = 0; j < individuals.length; j++) {
                    const other = individuals[j];
                    if (other !== org && other.type === 'predator' && other.alive) {
                ...
                if (neighborPreds > 2) {
                    isStressedPredator = true;
                    cost *= 2.0; // Double energy drain due to fighting and resource stress
                }
            }
            ...
            // Block splitting for predators experiencing territorial overcrowding stress
            if (org.energy >= splitThreshold && (!isStressedPredator)) {
                births++;
    ...
    function animate() {
        ...
        // Physics (eating and collisions) are handled exclusively in simulateGeneration() 
        // via SUBSTEPS to ensure deterministic physics that scale correctly with fast-forwarding.
