...
class Deer:
    ...
    def eat(self, env_state, config):
        ...
        for g in nearby_grass:
            if not g.eaten:
                # Verify exact distance to grass
                if (self.x - g.x)**2 + (self.y - g.y)**2 < config.deer_eat_range**2:
                    self.food_eaten += 1
                    self.days_starving = 0  # Reset starvation counter
                    g.eaten = True  # Mark grass as consumed
                    break  # Only eat one grass per tick

    def reproduce(self, config):
        """
        Handles reproduction logic. Only mature deer can reproduce.
        :param config: Configuration object.
        :return: A list containing a new Deer object if reproduction occurred, otherwise empty list.
        """
        if self.age < 37.5 or self.age >= 112.5:
            return []  # Juveniles and Adults (Old) do not reproduce
            
        if self.food_eaten >= config.deer_food_req:
            # 20% chance to reproduce when food requirement is met
            if random.random() <= 0.20:
                self.food_eaten -= config.deer_food_req  # Consume food for reproduction
                # Spawn offspring near parent
                return [Deer(self.x + random.uniform(-10, 10), self.y + random.uniform(-10, 10))]
        return []
    ...
    def daily_update(self, config):
        ...
        if self.age >= 37.5 and self.days_starving > config.deer_starvation_days:
            self.dead = True
