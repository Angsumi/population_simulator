...
class Lion:
    ...
    def eat(self, env_state, config):
        ...
        for d in nearby_deer:
            ...
            # If the deer is within lethal range
            if (self.x - d.x)**2 + (self.y - d.y)**2 < config.lion_eat_range**2:
                # 70% chance to successfully kill and eat the deer
                if random.random() <= 0.70:
                    self.food_eaten += 1
                    self.days_since_last_meal = 0  # Reset starvation counter
                    d.dead = True  # The deer is killed
                else:
                    # Failed hunt: Apply cooldown and scatter randomly
                    self.hunt_cooldown = 30
                    self.vx = random.uniform(-2, 2)
                    self.vy = random.uniform(-2, 2)
                break  # Only interact with one deer per tick

    def reproduce(self, config):
        """
        Handles reproduction logic based on food consumed. Only mature lions reproduce.
        :param config: Configuration object.
        :return: A list containing a new Lion object if reproduction occurred, otherwise empty list.
        """
        if self.age < 62.5 or self.age >= 187.5:
            return []  # Juveniles and Adults (Old) do not reproduce
            
        # Lions require more food to reproduce than deer typically
        if self.food_eaten >= config.lion_food_req:
            # 20% chance to reproduce when requirement met
            if random.random() <= 0.20:
                self.food_eaten -= config.lion_food_req  # Expend food to reproduce
                return [Lion(self.x + random.uniform(-10, 10), self.y + random.uniform(-10, 10))]
        return []
    ...
    def daily_update(self, config):
        ...
        if self.age >= 62.5 and self.days_since_last_meal > config.lion_starvation_days:
            self.dead = True
