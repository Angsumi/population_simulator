...
class Deer:
    ...
    def reproduce(self, config):
        if self.food_eaten >= config.deer_food_req:
            if random.random() <= 0.20:
                self.food_eaten -= config.deer_food_req
    ...
    def daily_update(self, config):
        ...
        self.age += 1
        if self.days_starving > config.deer_starvation_days:
            self.dead = True
        elif self.age > config.deer_max_age:
            days_over = self.age - config.deer_max_age
