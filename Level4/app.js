document.addEventListener('DOMContentLoaded', () => {
    ...
    class Organism {
        constructor(x, y, age = 0, energy = 100) {
            this.x = x;
            this.y = y;
        ...
        update() {
            // Wander (More active exploration in Level 3.1.1/4)
            this.vx += (Math.random() - 0.5) * 0.65;
            this.vy += (Math.random() - 0.5) * 0.65;

            // Food Sensing steering: steer towards nearest food within 60px
            if (foodItems && foodItems.length > 0) {
                let nearestFood = null;
                let minDistSq = 60 * 60; // sensory range limit (60 pixels)
                ...
                if (nearestFood) {
                    ...
                    if (dist > 0.1) {
                        // Apply a steering force towards the food
            ...
            // Damping (Slightly less damping to preserve momentum)
            this.vx *= 0.965;
            this.vy *= 0.965;
    ...
    function spawnInitialPopulation(count) {
        ...
        // Spawn initial organisms with energy = 100
        ...
        const initialFood = Math.round(count * 1.5);
    ...
    // ─────────────────────────────────────────────────────────
    function simulateGeneration(baseFoodSpawnRate, seasonLength, survivalCost) {
        const pop = individuals.length;
        if (pop <= 0) return { births: 0, deaths: 0, perCapitaBirthProb: 0, perCapitaDeathProb: 0 };

        const nutritiveValue = 45; // Fixed food value

        // 1. Calculate current seasonal food spawn rate
        // We use sine wave modulation: baseRate * (1 + sin(2*pi*t / seasonLength))
        // High spawn rate in Summer (sine near 1), low in Winter (sine near -1)
        ...
        for (let i = 0; i < seasonalSpawnRate; i++) {
            if (foodItems.length < maxFood) {
                // 75% chance to spawn near existing food (dispersal), 25% random colonization
                if (foodItems.length > 0 && Math.random() < 0.75) {
                    const parent = foodItems[Math.floor(Math.random() * foodItems.length)];
                    const angle = Math.random() * Math.PI * 2;
                    const dist = Math.random() * 20 + 8; // spawn 8-28px away
        ...
        for (let step = 0; step < SUBSTEPS; step++) {
            for (let i = 0; i < pop; i++) {
                ...
                // Move organism (which now steers towards food!)
                org.update();

                // Check food collision (radius + 8 for sensory range)
                ...
                for (let j = foodItems.length - 1; j >= 0; j--) {
                    ...
                    if (distSq < eatRadius * eatRadius) {
                        // Consume food
                        org.energy = Math.min(250, org.energy + nutritiveValue);
        ...
        for (let i = 0; i < pop; i++) {
            ...
            // Check for birth by splitting (energy threshold = 180)
            const splitThreshold = 180;
            if (org.energy >= splitThreshold) {
                ...
                // Child has same energy as parent (split equally)
                const child = new Organism(childX, childY, 0, org.energy);
        ...
        const perCapitaBirthProb = births / pop;
        const perCapitaDeathProb = deaths / pop;
    ...
    // Calculate theoretical emergent carrying capacity (average across seasonal cycles)
    function getEmergentK() {
        const spawnRate = parseFloat(inputFoodSpawnRate.value) || 30;
        const nutritiveValue = 45; // Fixed energy value
        const survivalCost = parseFloat(inputSurvivalCost.value) || 10;
    ...
    function startSimulation() {
        ...
        const p0 = Math.max(1, Math.round(parseFloat(inputInitialPop.value) || 80));
        const foodSpawnRate = parseFloat(inputFoodSpawnRate.value) || 30;
    ...
    function runGenerationStep() {
        ...
        if (currentGeneration === 0) {
            ...
            const result = simulateGeneration(foodSpawnRate, seasonLength, survivalCost);
