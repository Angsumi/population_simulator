document.addEventListener('DOMContentLoaded', () => {
    ...
    const inputNutritiveValue = document.getElementById('nutritiveValue');
    ...
    class Organism {
        ...
        update() {
            // Wander (More active exploration in Level 3.1.1)
    ...
    // ─────────────────────────────────────────────────────────
    function simulateGeneration(foodSpawnRate, nutritiveValue, survivalCost) {
        const pop = individuals.length;
        if (pop <= 0) return { births: 0, deaths: 0, perCapitaBirthProb: 0, perCapitaDeathProb: 0 };

        // 1. Spawn new food dots
        const maxFood = 1200; // Capped for performance and visual clarity
        ...
        // This ensures they move around and eat food dynamically, resolving the "not moving enough / starving" issue.
        const SUBSTEPS = 10;
        for (let step = 0; step < SUBSTEPS; step++) {
            for (let i = 0; i < pop; i++) {
                ...
                // Check food collision (radius + 8 for a slightly larger sensory range)
    ...
    function getEmergentK() {
        const spawnRate = parseFloat(inputFoodSpawnRate.value) || 25;
        const nutritiveValue = parseFloat(inputNutritiveValue.value) || 40;
        const survivalCost = parseFloat(inputSurvivalCost.value) || 12;
    ...
    function startSimulation() {
        ...
        inputNutritiveValue.disabled = true;
        ...
        const p0 = Math.max(1, Math.round(parseFloat(inputInitialPop.value) || 50));
        const foodSpawnRate = parseFloat(inputFoodSpawnRate.value) || 25;
        const nutritiveValue = parseFloat(inputNutritiveValue.value) || 40;
        const survivalCost = parseFloat(inputSurvivalCost.value) || 12;
    ...
    function runGenerationStep() {
        ...
        if (currentGeneration === 0) {
            ...
            const result = simulateGeneration(foodSpawnRate, nutritiveValue, survivalCost);
    ...
    function resetSimulation() {
        ...
        inputNutritiveValue.disabled = false;
