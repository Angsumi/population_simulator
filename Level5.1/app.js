document.addEventListener('DOMContentLoaded', () => {
    ...
    class Organism {
        ...
        update() {
            ...
            if (this.type === 'herbivore') {
                // Steer towards nearest grass within 60px
                ...
                // Predator Satiation: Satiated predators (energy > 140) wander peacefully, giving prey recovery time
                const isSatiated = this.energy > 140;
                ...
                if (!isSatiated) {
                    ...
                    for (let i = 0; i < individuals.length; i++) {
                        ...
                        if (other.type === 'herbivore' && other.alive) {
                            // Prey Camouflage Refuge: Prey in the central forest (35%-65% width) are invisible to predators
                            const inRefuge = (other.x > canvas.width * 0.35 && other.x < canvas.width * 0.65);
                    ...
                    for (let j = 0; j < individuals.length; j++) {
                        ...
                        if (prey.alive && prey.type === 'herbivore') {
                            ...
                            if (distSq < eatRadius * eatRadius) {
                                prey.alive = false;
                                prey.energy = 0;
                                pred.energy = Math.min(300, pred.energy + predatorNutritiveValue);
    ...
    function animate() {
        ...
        // Draw Prey Refuge Zone (central Camouflage Forest)
        ctxCanvas.fillStyle = 'rgba(16, 185, 129, 0.03)';
        ctxCanvas.fillRect(canvas.width * 0.35, 5, canvas.width * 0.3, canvas.height - 10);
        ctxCanvas.strokeStyle = 'rgba(16, 185, 129, 0.08)';
        ctxCanvas.lineWidth = 1;
        ctxCanvas.strokeRect(canvas.width * 0.35, 5, canvas.width * 0.3, canvas.height - 10);
        
        ctxCanvas.fillStyle = 'rgba(16, 185, 129, 0.15)';
        ctxCanvas.font = 'bold 9px Outfit, sans-serif';
        ctxCanvas.fillText('CAMOUFLAGE FOREST (SAFE ZONE)', canvas.width * 0.35 + 10, 20);
