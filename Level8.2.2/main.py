...
def main():
    ...
    for day in range(1, total_days + 1):
        for _ in range(tick_rate):
            ...
            d_juv = sum(1 for d in sim.deer if d.age < 37.5)
            d_mat = sum(1 for d in sim.deer if 37.5 <= d.age < 112.5)
            d_adu = sum(1 for d in sim.deer if d.age >= 112.5)
            
            l_juv = sum(1 for l in sim.lions if l.age < 62.5)
            l_mat = sum(1 for l in sim.lions if 62.5 <= l.age < 187.5)
            l_adu = sum(1 for l in sim.lions if l.age >= 187.5)
