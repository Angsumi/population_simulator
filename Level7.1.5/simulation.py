...
class Simulation:
    ...
    def step(self):
        ...
        for d in self.deer:
            ...
            born_deer.extend(d.eat_and_reproduce(env_state, self.config))
                        
        for l in self.lions:
            if l.dead: continue
            l.update(env_state, self.config)
            born_lions.extend(l.eat_and_reproduce(env_state, self.config))
        ...
        if self.tick_count >= self.config.tick_rate:
            ...
            self.grass = [g for g in self.grass if not g.eaten]
            self.deer = [d for d in self.deer if not d.dead]
            self.lions = [l for l in self.lions if not l.dead]
